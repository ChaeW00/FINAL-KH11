...
public class FreeInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, 
		...
		FreeDto freeDto = freeRepo.selectOne(freeNo);
		String writerId = freeDto.getFreeWriter();
		...
		if(isAdmin || isOwner) {
			if(request.getRequestURI().equals(request.getContextPath() + "/free/delete") || 
					request.getRequestURI().equals(request.getContextPath() + "/free/write")) {
				return true;
			}
		}
