	...
	$(function(){
		...
		$(document).on("click", ".btn-reply-submit", function(){
			...
			.then(function(response){
				...
				var img_reply_member_img = $("<img>").attr("class", "img-member-profile member-profile").attr("src", "${pageContext.request.contextPath}/attachment/download/teamMember?attachmentNo=" + response.data.memberAttachmentNo).attr("data-writerno", response.data.replyWriterNo);
		...
		$(document).on("click", ".btn-reply-more", function(){
			...
			.then(function(response){
				...
				for(var i = 0 ; i < response.data.replyList.length ; i ++){
					...
					// 2-1) span 태그
					var span_reply_member_img_container = $("<span>").attr("class", "div-reply-member-profile");
					var img_reply_member_img = $("<img>").attr("class", "img-member-profile member-profile").attr("src", "${pageContext.request.contextPath}/attachment/download/teamMember?attachmentNo=" + response.data.replyList[i].memberAttachmentNo).attr("data-writerno", response.data.replyList[i].replyWriterNo);
					var span_reply_member_img = span_reply_member_img_container.append(img_reply_member_img);
				...
				if(reply_list_state == "1") { // 이미 조회한 상태라면
					...
					.then(function(response){
						...
						if(response.data.length != 0) {
							...
							var img_reply_insert = $("<img>").attr("class", "img-member-profile member-profile reply-profile").attr("src", "${pageContext.request.contextPath}/attachment/download/teamMember?attachmentNo=" + memberAttachmentNo);
...
<script type="text/javascript">
	...
	$(function(){
		...
		// - 확인 버튼 클릭시 비동기로 게시글 등록 요청 전송
	 	$(document).on("click", ".modal-insert-submit", function(e){
			...
			var teamNo = $("#div-member-info").data("teamno");
	        // 태그를 포함하여 내용 지정
			...
			.then(function(response){
				...
				var divtop_img_member = $("<img>").attr("class", "img-member-profile member-profile").attr("src", "${pageContext.request.contextPath}/attachment/download/teamMember?attachmentNo=" + response.data.memberAttachmentNo).attr("data-writerno", response.data.boardWriterNo); // 임시 주소
        ...
        $(document).on("click", ".editor-open-edit", function(e){
            ...
            $(".modal-edit-submit").off().click(function(e){ // off()가 없으면 이상하게 클릭 이벤트가 2번 발생함(?)
    			...
    			// 현재 접속중인 소모임 페이지의 소모임 번호
    			var boardteamNo = $("#div-member-info").data("teamno");
                // 태그를 포함하여 내용 지정
	...
	$(function(){
    	...
    	$(document).on("click", ".btn-search-submit", function(){
			...
			var url = "${pageContext.request.contextPath}/rest_board/search?teamNo=" + teamNo + "&memberNo=" + memberNo + "&p=" + p + "&keyword=" + keyword;
			...
			.then(function(response){
				...
				for(var i = 0 ; i < response.data.boardList.length ; i ++){
					...
					var divtop_span = $("<span>").attr("class", "div-member-profile");
					var divtop_img_member = $("<img>").attr("class", "img-member-profile member-profile").attr("src", "${pageContext.request.contextPath}/attachment/download/teamMember?attachmentNo=" + response.data.boardList[i].memberAttachmentNo).attr("data-writerno", response.data.boardList[i].boardWriterNo); // 임시 주소
					var divtop_img = divtop_span.append(divtop_img_member);
	...
	$(window).scroll(_.debounce(function(){
		...
		if(percentage > 80) {
			...
			var url;
			if(keyword != "") {
				url = "${pageContext.request.contextPath}/rest_board/search?teamNo=" + teamNo + "&memberNo=" + memberNo + "&keyword=" + keyword + "&p=" + p;
			} else {
				url = "${pageContext.request.contextPath}/rest_board/search?teamNo=" + teamNo + "&memberNo=" + memberNo + "&p=" + p;
			}
	...
	function loadList(){
		...
		var url = "${pageContext.request.contextPath}/rest_board/search?teamNo=" + teamNo + "&memberNo=" + memberNo + "&p=" + p;
